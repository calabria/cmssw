...
void TestCudaService::CUDAAutolaunchManagedTest()
{
  ...
  // Auto launch config
  cudaConfig::ExecutionPolicy execPol((*cuSerPtr)->configureLaunch(n, longKernel));
...
void TestCudaService::CUDAAutolaunch2Dconfig()
{
  ...
  cudaConfig::ExecutionPolicy execPol((*cuSerPtr)->configureLaunch(n*m, matAddKernel));
...
void TestCudaService::originalKernelTest(){
  ...
  auto execPol= (*cuSerPtr)->configureLaunch(meanExp, originalKernel);
