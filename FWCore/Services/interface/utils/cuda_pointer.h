/*! Defines `cudaPointer` smart pointers for CUDA data arrays.
  cudaPointer is a template class with 2 specializations: 1 for single arithmetic
  types (int, float, ...) and 1 for arrays (int[], float[], ...) in the model of
  `std::unique_ptr`.

  File contents:
  1. class cudaPtrBase
  2. class cudaPointer<T[]>
  3. class cudaPonter<T>
  4. [FUTURE] struct ContainsCudaPointer
  5. cudaPointer<T[]> template method definitions
  6. cudaPointer<T>   template method definitions
...
public:
  ...
  */
  ...
  void reset(int elementN, Attachment newAttachment= host){
    if (attachment_ == noAlloc){
      attachment_= newAttachment;
  ...
  T& operator[](int idx) const{
    if (attachment_ == single)
      throw new std::runtime_error("Illegal cudaPointer access from CPU, "
